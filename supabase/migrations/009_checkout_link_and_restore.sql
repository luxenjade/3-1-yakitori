-- checkout_order を更新: 仮注文IDを注文に紐付ける
...
BEGIN
  ...
  INSERT INTO orders (total_price, payment_method, status, order_source, source_temporary_order_id)
  VALUES (v_total, p_payment_method, 'pending', p_order_source, v_temp_id)
...
-- キッチン: 誤って完了にした注文を調理中に戻す
CREATE OR REPLACE FUNCTION restore_order_to_cooking(p_order_id UUID)
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  UPDATE orders
  SET status = 'cooking', pickup_token = NULL, pickup_expires_at = NULL, pickup_used_at = NULL
  WHERE id = p_order_id;
...
REVOKE ALL ON FUNCTION restore_order_to_cooking(UUID) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION restore_order_to_cooking(UUID) TO authenticated;